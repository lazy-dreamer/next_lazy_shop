...
import {ICheckoutForm, IOrder, saveUserOrders} from "../../services/firebase/orders";
...
export const CheckoutSection:React.FC<Props> = ({className=''}) => {
  ...
  const {userInfo, cart, setIsCheckout, orders, setOrders, setUserInfo} = useUserStore()
  const {
    register,
    handleSubmit,
    reset,
  ...
  const onSubmit = (data:ICheckoutForm)=> {
    if (userInfo?.name.length == 0 && userInfo.surname.length == 0 && userInfo.phone.length == 0 && userInfo.email.length == 0) {
      let dta = {
        ...data
      };
      delete dta.comment;
