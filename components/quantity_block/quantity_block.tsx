...
export const QuantityBlock:React.FC<Props> = ({className='', productId, quantity= 1}) => {
  ...
  return <div className={`${className && className} ${s.block}`}>
    <button type='button' className={`${s.btn} ${s.minus} ${quantity==1 && 'disabled'}`} onClick={() => quantityHandler('minus')} />
    <input type="text" value={quantity} onChange={() => console.log('changed')} />
    <button type='button' className={`${s.btn} ${s.plus}`} onClick={() => quantityHandler('plus')} />
  </div>;
}