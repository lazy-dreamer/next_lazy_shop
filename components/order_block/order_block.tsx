...
export const OrderBlock: React.FC<Props> = ({
  ...
  const time =
    order.orderTime.substring(order.orderTime.indexOf(":") + 1).length > 1
      ? order.orderTime
      : order.orderTime.substring(0, order.orderTime.indexOf(":") + 1) +
        "0" +
  ...
  return (
    <div className={`${className && className} ${s.block} frame`}>
      <div className={s.title}>
        ...
        <p>
          Order from - {order.orderDate} ({time})
        </p>
      </div>
      ...
      <div className={s.delivery}>
        ...
        <div>
          <p>
            <b>Name:</b> {`${order.orderInfo.name} ${order.orderInfo.surname}`}
          </p>
          <p>
            <b>Phone:</b> {`${order.orderInfo.phone}`}
          </p>
          <p>
            <b>Address:</b>{" "}
            {`${order.orderInfo.city}, ${order.orderInfo.street} ${order.orderInfo.building}, apt.${order.orderInfo.apartment}`}
          </p>
          {order.orderInfo.comment && order.orderInfo.comment.length > 0 && (
            <p>
              <b>Additional comment: </b> {order.orderInfo.comment}
            </p>
          )}
        </div>
      </div>
