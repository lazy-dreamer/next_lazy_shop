...
const fetchProducts = async (query: string): Promise<IProduct[]> => {
  const response = await axios(`${process.env.NEXT_PUBLIC_API_URL}${ApiRoutes.PRODUCTS}?title=${query}`);
...
export const ProductSearch:React.FC<Props> = ({className=''}) => {
  ...
  return <div className={`${className && className} ${s.block} frame`} ref={wrapperRef}>
    <form className={s.search} autoComplete={'off'}>
      <input type="text" value={search} placeholder={'Search product...'} name={'search input'} onChange={(e: React.ChangeEvent<HTMLInputElement>) => onSearchChange(e)} />
    </form>
    {
