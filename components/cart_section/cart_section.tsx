    ...
    <div className="screen_content">
      <div className={s.sides}>
        <div className={s.blocks}>
          ...
          {
            cart.length > 0 ?
              cart.map((item:ICartItem) => <CartItem key={item.product.id} item={item} />)
            : isCartLoaded && <Title text={'Your cart is empty :('} size={'md'} className={'text_center'} />
          }
        </div>
        <div className={s.aside}>
          <Title text={cart.length > 0 ? 'Order info' : 'No order info'} size={'xs'} />
          {
            cart.length > 0 && <>
              <CartInfo />  
              <Link href={'/checkout'} className={'main_btn min_wide'}>Go to checkout</Link>
            </>
          }
        </div>
      </div>
