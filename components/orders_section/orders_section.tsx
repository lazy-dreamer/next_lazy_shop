    ...
    <div className="screen_content">
      <div className='user_info_sides'>
        ...
        <div className="user_info_side">
          {isOrdersLoaded && (orders.length>0 ? <Title text={'Your orders'} size={'lg'} /> : <Title text={'You have no orders yet :('} size={'lg'} />)}
          {
            orders.map((order, index) => <OrderBlock index={index+1} key={index} order={order} />)
          }
        </div>
      </div>
    </div>
